...
export const themeColors = [
  { name: '极光紫蓝', color: '#4F46E5' },
  { name: '星际深蓝', color: '#2563EB' },
  { name: '碧波翡翠', color: '#059669' },
  { name: '深邃黑', color: '#111827' },
  { name: '极客蓝', color: '#2E5CF6' },
...
export const useThemeStore = defineStore('theme', () => {
  ...
  // 主题色（默认极光紫蓝 #4F46E5）
  const savedPrimaryColor = localStorage.getItem('layout-primary-color')
  const primaryColor = ref<string>(
    (savedPrimaryColor && savedPrimaryColor !== '#111827') ? savedPrimaryColor : '#4F46E5'
  )
